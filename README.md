...
- **整理模式**：左下角「整理」按钮进入整理，当前分类图标可单选 / 多选，批量移动到任意分类或删除
- **管理员登录**：未登录仅可浏览 / 跳转 / 复制链接；登录后可上传、设置、整理、删除（初始 admin / password，可改密）
...
（需先登录，见「管理员登录」）点右上角「上传」→ 点击或拖拽图片 → 选择分类（可新建），可**填写自定义跳转链接（可选）**→ 开始上传，自动刷新后即生效。填写链接的图标点击后直接打开该链接，不填则点击复制图标链接。服务模式下图片与记录均已写入服务器。

**删除图标**：图标卡片**左上角有删除按钮**（悬停显示，仅管理员可见），点击确认即删除——网页上传的图标同步删除服务器 `icons/` 下的图片文件和 `uploads.json` 记录；`data.js` 默认图标删除后从页面隐藏（配置文件保留，重新上传同名文件或编辑 `data.js` 可恢复）。
...
- 初始账户 `admin`，初始密码 `password`（服务模式存于 `data/auth.json`，加盐哈希，不存明文）
- 登录后顶栏显示「退出 admin」，点击即退出
- 设置 →「账号安全」可修改密码（服务模式对所有访问者生效；静态双击打开时密码保存在当前浏览器，仅本机生效）
