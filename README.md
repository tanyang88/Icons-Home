...
- **上传自定义链接**：上传弹窗可为每个图标填写跳转链接，上传后点击图标直接打开
...
├── data/settings.json   # 站点设置：标题 / 隐藏的分类 / 隐藏的图标（服务模式自动维护）
...
点右上角「上传」→ 点击或拖拽图片 → 选择分类（可新建），可**填写自定义跳转链接（可选）**→ 开始上传，自动刷新后即生效。填写链接的图标点击后直接打开该链接，不填则点击复制图标链接。服务模式下图片与记录均已写入服务器。
...
- `data/categories.json`：网页端自定义分类（服务模式自动维护）
- `data/settings.json`：站点设置——标题、隐藏的基础分类、隐藏的基础图标（服务模式自动维护）
