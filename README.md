...
- **分类自由**：网页「设置」面板可直接**添加 / 删除自定义分类**（服务模式持久化到服务器，所有人可见）；data.js 基础分类自动统计数量
- **图标删除**：网页上传的图标悬停显示删除按钮，一键删除（服务模式同步删除服务器 `icons/` 文件与记录）
- **自动排序**：图标默认按名称排序——英文按字母 a-z，中文按拼音
...
**删除图标**：网页上传的图标卡片**左上角有删除按钮**（悬停显示），点击确认即删除——服务模式同步删除服务器 `icons/` 下的图片文件和 `uploads.json` 记录。`data.js` 基础图标不显示删除按钮，需在 `data.js` 中管理。
...
- 分类列表点「×」删除自定义分类；删除时该分类下的上传图标自动移到「其他」
- `data.js` 基础分类标注「基础」，不可在网页删除（请用下面的方式编辑 `data.js`）
...
分类栏自动按此渲染并统计数量，图标默认按名称 a-z（中文按拼音）排序。
